...
def __cal_coord_by_ratio_adjustment(points, temp_startline, temp_carspan, test_startline, test_carspan, axis=1):
...
def __cal_new_pts(pt, temp_pts, first_ref, ref_segl, first_cur, cur_segl, main_axis, minor_axis_poly_func):
...
class Locator:
    def __init__(self,  qtrain_info, local_params, itemInfo, device, axis=1, logger=None):
        ...
        self.test_startline = self.qtrain_info.test_train.startline
        self.test_endline = self.qtrain_info.test_train.endline 
        self.test_carspan = self.test_endline - self.test_startline
    ...
    def update_test_img(self, img, img_w, img_h):
        self.test_img = img
        self.img_w = img_w
        self.img_h = img_h

    def locate_anchors_yolo(self):
        anchor_bboxes = bbox_formater(self.itemInfo["anchors"])

        # load model 
        model = YoloInfer(self.local_params.locator.model_path, 
                self.device, imgsz=self.local_params.locator.imgsz,
                logger=self.logger)
        label_translator = self.local_params.locator.label_translator

        new_anchor_bboxes = [] 
        for idx, bbox in enumerate(anchor_bboxes):
            # get proposal rect in frame points
            bbox.proposal_rect = __cal_coord_by_ratio_adjustment(bbox.temp_rect, self.temp_startline, self.temp_carspan, 
            ...
            propoal_rect_p = frame2rect(bbox.proposal_rect, self.test_startline, 
                            self.img_h, self.img_w, start_minor_axis_fp=0, axis=self.axis)
            # proposal rect image 
            proposal_rect_img = crop_segmented_rect(self.test_img, propoal_rect_p)
            proposal_rect_img = cv2.cvtColor(proposal_rect_img, cv2.COLOR_GRAY2BGR)

            # infer
            temp_outputs = model.infer(proposal_rect_img, conf_thres=self.local_params.locator.conf_thres,
                        iou_thres=self.local_params.locator.iou_thres)
            ...
            if len(temp_outputs) > 0:
                ...
                bbox.conf_thres = self.local_params.locator.conf_thres
                new_anchor_bboxes.append(bbox)

        return new_anchor_bboxes

    def _generate_anchor_img(self, save_path, aux="anchor"):
        ...
        for idx, bbox in enumerate(anchor_bboxes):
            ...
            # proposal rect image 
            proposal_rect_img = crop_segmented_rect(self.test_img, propoal_rect_p)
            fname = "{}_{}_{}_{}_{}_{}_{}.jpg".format(
                aux, self.qtrain_info.minor_train_code, self.qtrain_info.train_num, 
                self.qtrain_info.train_sn, self.channel, self.carriage, bbox.name)
            fname = save_path / fname
            cv2.imwrite(str(fname), proposal_rect_img)
            print(f">>> {fname}.")
    ...
    def _auto_update_minor_axis_affine_transform_matrix(self, x, y, poly_order=1):
        ...
        z = np.polyfix(x, y, poly_order) 
        minor_axis_poly_func = np.poly1d(z)
        return minor_axis_poly_func

                
    def locate_bboxes_according2anchors(self, item_bboxes, anchor_bboxes):
        if not item_bboxes:
            return []
        ...
        for anchor in anchor_bboxes:
            temp_anchor_points.append(anchor.temp_rect[0]) # left top pt [x, y]
            temp_anchor_points.append(anchor.temp_rect[1]) # right bottom pt [x, y]
        ...
        if self.local_params.locator.minor_axis_affine_maxtrix:
            minor_axis_poly_func = self._update_minor_axis_affine_transform_matrix(self.local_params.locator.minor_axis_affine_maxtrix)
        elif self.local_params.locator.auto_minor_axis_adjust:
        ...
        for i in range(seg_cnt):
            ...
            for j, bbox in enumerate(item_bboxes):
                ...
                if first_ref <= pt0 <= second_ref:
                    bbox.proposal_rect[0] = __cal_new_pts(pt0, bbox.temp_rect[0], first_ref, ref_segl,
                                            first_cur, cur_segl, main_axis, minor_axis_poly_func)

                if first_ref <= pt1 <= second_ref:
                    bbox.proposal_rect[1] = __cal_new_pts(pt1, bbox.temp_rect[1], first_ref, ref_segl,
                                            first_cur, cur_segl, main_axis, minor_axis_poly_func)
